...
func (s *snapshotter) mounts(ctx context.Context, snap storage.Snapshot, checkKey string) ([]mount.Mount, error) {
	var options []string
...
func (s *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) (_ storage.Snapshot, err error) {
	var (
...
func (s *snapshotter) Prepare(ctx context.Context, key, parent string, opts ...snapshots.Opt) ([]mount.Mount, error) {
	snap, err := s.createSnapshot(ctx, snapshots.KindActive, key, parent, opts)
	if err != nil {
		return nil, err
	}
	...
	if target, ok := base.Labels[targetSnapshotLabel]; ok {
		...
		if err := s.prepareRemoteSnapshot(lCtx, key, base.Labels); err != nil {
			...
			err := s.commit(ctx, true, target, key, append(opts, snapshots.WithLabels(base.Labels))...)
	...
	return s.mounts(ctx, snap, parent)
}

func (s *snapshotter) View(ctx context.Context, key, parent string, opts ...snapshots.Opt) ([]mount.Mount, error) {
	snap, err := s.createSnapshot(ctx, snapshots.KindView, key, parent, opts)
	if err != nil {
		return nil, err
	}
	return s.mounts(ctx, snap, parent)
...
func (s *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) error {
	return s.commit(ctx, false, name, key, opts...)
}

func (s *snapshotter) commit(ctx context.Context, isRemote bool, name, key string, opts ...snapshots.Opt) error {
...
func (o *snapshotter) prepareRemoteSnapshot(ctx context.Context, key string, labels map[string]string) error {
	...
	// TODO: Implement remote mount operation
	return nil
}
...
func convertDirToErofs(ctx context.Context, layerBlob, upperDir string) error {
	err := ConvertErofs(ctx, layerBlob, upperDir, nil)
	if err != nil {
		return err
	}
