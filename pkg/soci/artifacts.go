...
func NewDB(path string) (*ArtifactsDb, error) {
	once.Do(func() {
		...
		if err := os.MkdirAll(dir, 0700); err != nil {
			log.G(context.Background()).WithError(err).WithField("path", dir).Error("Cannot create directory for artifacts db")
			return
		}
...
func (db *ArtifactsDb) SyncWithLocalStore(ctx context.Context, blobStore Store, blobStorePath string, cs content.Store) error {
...
func (db *ArtifactsDb) RemoveOldArtifacts(ctx context.Context, blobStore Store) error {
...
func (db *ArtifactsDb) addNewArtifacts(ctx context.Context, blobStorePath string, cs content.Store) error {
	...
	return filepath.WalkDir(blobStorePath, func(path string, d fs.DirEntry, err error) error {
		...
		if sociIndex.ArtifactType != SociIndexArtifactTypeV1 {
			return nil
		}
...
func loadArtifact(artifactBkt *bolt.Bucket, digest string) (*ArtifactEntry, error) {
	...
	size, err := decodeInt(encodedSize)
	if err != nil {
		return nil, err
	}
	createdAt := time.Time{}
...
func putArtifactEntry(artifacts *bolt.Bucket, ae *ArtifactEntry) error {
	...
	sizeInBytes, err := encodeInt(ae.Size)
	if err != nil {
		return err
	}
