...
func NewIndexBuilder(contentStore content.Store, blobStore Store, opts ...BuilderOption) (*IndexBuilder, error) {
	cfg := &builderConfig{
	...
	if cfg.artifactsDb == nil {
		var err error
		cfg.artifactsDb, err = NewDB(ArtifactsDbPath(DefaultArtifactsDbPath))
		if err != nil {
			return nil, err
		}
	}
	...
	return &IndexBuilder{
		...
		blobStore:    blobStore,
		config:       cfg,
		ztocBuilder:  ztoc.NewBuilder(DefaultBuildToolIdentifier),
	}, nil
}
...
func (b *IndexBuilder) build(ctx context.Context, img images.Image, buildCfg buildConfig) (*IndexWithMetadata, error) {
	...
	if joinErr != nil {
		return nil, fmt.Errorf("errors encountered while building soci layers: %w", joinErr)
	}
...
func (b *IndexBuilder) writeSociIndex(ctx context.Context, indexWithMetadata *IndexWithMetadata, gcRoot bool) (ocispec.Descriptor, error) {
	...
	if gcRoot {
		err = LabelGCRoot(ctx, b.blobStore, desc)
		if err != nil {
			return ocispec.Descriptor{}, fmt.Errorf("cannot apply garbage collection label to index %s: %w", desc.Digest.String(), err)
		}
	}
	err = LabelGCRefContent(ctx, b.blobStore, desc, "config", defaultConfigDescriptor.Digest.String())
	if err != nil {
	...
	for i, blob := range indexWithMetadata.Index.Blobs {
		err = LabelGCRefContent(ctx, b.blobStore, desc, "ztoc."+strconv.Itoa(i), blob.Digest.String())
		if err != nil {
