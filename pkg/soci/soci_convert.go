...
func (b *IndexBuilder) Convert(ctx context.Context, img images.Image, opts ...ConvertOption) (*ocispec.Descriptor, error) {
	...
	for _, opt := range opts {
		if err := opt(&convertCfg); err != nil {
			return nil, err
		}
	}
	convertCfg.platforms = dedupePlatforms(convertCfg.platforms)
	...
	for i, desc := range ociIndex.Manifests {
		err := LabelGCRefContent(ctx, b.blobStore, ociIndexDesc, fmt.Sprintf("m.%d", i), desc.Digest.String())
		if err != nil {
			return nil, err
		}
	}
	if convertCfg.gcRoot {
		err := LabelGCRoot(ctx, b.blobStore, ociIndexDesc)
		if err != nil {
			return nil, err
		}
	}
...
func (b *IndexBuilder) newOciIndex(ctx context.Context, img images.Image) (ocispec.Index, error) {
	if images.IsIndexType(img.Target.MediaType) {
		...
		err = validateMediaType(b, img.Target.MediaType)
		if err != nil {
			return ocispec.Index{}, err
		}

		var ociIndex ocispec.Index
		if err := json.Unmarshal(b, &ociIndex); err != nil {
			return ocispec.Index{}, err
		}
